...
async function fetchReport() {
  ...
  const daily = (body.data ?? []).map((bucket) => { let tokens=0; let cacheTokens=0; for (const r of bucket.results ?? []) { if (r.model) models.add(r.model); const cacheRead=r.cache_read_input_tokens||0; const cacheWrite=Object.values(r.cache_creation||{}).reduce((a,b)=>a+(typeof b === "number" ? b : 0),0); const input=r.uncached_input_tokens||0; const output=r.output_tokens||0; cacheTokens += cacheRead + cacheWrite; tokens += input + output + cacheRead + cacheWrite; } totalTokens += tokens; return {date:bucket.ending_at || new Date().toISOString(),tokens,cacheTokens}; });
  return {totalTokens,models:models.size,days:7,daily};
}
